...
export default function AICoachScreen() {
  ...
  const chatMutation = trpc.aiCoach.chat.useMutation({
    onError: (e: any) => {
      let msg = 'The coach hit capacity—try again in a moment.';
      
      // Map network errors to friendly messages
      if (e?.message?.includes('fetch') || e?.message?.includes('network')) {
        msg = 'Network error. Check API URL/CORS and try again.';
      } else if (e?.message?.includes('Failed to fetch') || e?.message?.includes('ERR_NETWORK')) {
        msg = 'Cannot reach the server. Check API URL/CORS configuration.';
      } else if (e?.message) {
        msg = e.message;
      }
